...
function App() {
  let cats = [
    {key: 1, name: 'Robin', age: 3, color: 'orange'},
    {key: 2, name: 'Simba', age: 5, color: 'white'},
    {key: 3, name: 'Peki', age: 2, color: 'gray'},
    {key: 4, name: 'Stepko', age: 19, color: 'Drko'},
    {key: 5, name: 'Frane', age: 9, color: 'black'},
    ]
    ...
    const [NewCat, insertNewCat] = useState(0)

    const toggleInsert = () => {
      NewCat ? insertNewCat(0) : insertNewCat(1)
    }
    if(NewCat){
      let catObj = {key: '?', name: newName, age: newAge, color: newColor}
      cats.push(catObj)
      console.log(cats)
    }
    ...
    if(newCat){
      catForm = (
        <form>
          ...
          <input onClick={toggleInsert} type="button" value="Add"></input> <input onClick={toggleNewCat} type='button' value='Exit'></input>
        </form>
      )
    }
